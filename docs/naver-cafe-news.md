...
**번호 규칙(2026-09-16 오너 확정)**: 카페 제목의 `#N`은 웹 소식 번호가 아니라, 카페에
이미 등록된 마지막 파킨온 소식 번호에 반드시 1을 더한다. 따라서 #12 웹 소식을 카페에
옮길 때 제목은 **`파킨온 소식 #7.`** 으로 시작한다.
...
- **대한민국 네이버 카페용 사진은 해외 인물이 보이는 사진을 쓰지 않는다**(2026-09-16
  오너 확정). 웹 글에 두 장이 있으면 인물이 보이지 않거나 얼굴·국적이 드러나지 않는
  이미지를 고른다. #12에는 의료진 히어로 대신 얼굴이 보이지 않는 발·다리 통증 사진
  (`sitting-feet-pain.jpg`)만 쓴다.
...
- [ ] 제목 = 카페 마지막 번호 + 1 (`#12 웹 글은 카페 #7`) + 웹 글 제목 **그대로**
