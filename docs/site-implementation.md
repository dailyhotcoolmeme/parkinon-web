...
**✅ 애드센스 신청 완료(2026-08-10)** — 오너가 로그인한 기존 AdSense 계정
(pub-2792582436871752, admin@ourmine.co.kr)에 **toolshere.app과 별개의 사이트**로
parkinon.com을 추가했다(오너 지시: "toolshere랑 겹치지 않게" — 새 계정을 만드는 게
아니라 같은 계정의 "사이트" 목록에 독립된 행으로 추가하는 것이 맞다). 소유권 확인은
Ads.txt 방식으로 했다(코드 스니펫 방식은 승인 전에 광고 스크립트를 미리 심게 되어
"광고 스크립트 0개"라는 방침 문구가 거짓이 됨 — 그래서 스크립트 없는 ads.txt 쪽을
골랐다). `site/public/ads.txt`에 `google.com, pub-2792582436871752, DIRECT,
f08c47fec0942fa0`를 추가해 배포했고, 실제 계정에서 소유권 확인·검토 요청까지 완료해
지금 "사이트의 광고 게재 가능 여부 검토 중" 상태다. toolshere.app 은 그대로 별개로
"준비 중" 상태 유지 — 서로 안 겹침.
...
3. 웹 방침(`src/pages/ko/privacy.astro`) **제2조·제4조 갱신** — "쿠키 없음"을 실제 사용 쿠키로,
   ...
   ✅ **문구는 이미 다 써서 그 파일 안에 주석으로 넣어놨다(2026-08-10)** — 제2조 `<p>`
   두 개 바로 아래, 제4조 `</table>` 바로 아래에 각각 `{/* ⚠️⚠️ ... */}` 주석으로
   교체용 HTML이 준비돼 있다. **이 시점에는 새로 쓰지 말고 그 주석 안의 내용을 실제
   위치로 옮기기만 하면 된다.** (오너 지시: "안 까먹게 꼭 넣어놔라")
4. 광고 태그 넣은 뒤 EEA 접속에서 배너가 실제로 뜨는지 확인
5. ✅ **한국 요구사항 조사 완료(2026-08-10)** — PIPC/방통위 "온라인 맞춤형 광고
   개인정보보호 가이드라인" 정부 원문 PDF는 파서로 못 읽어서(바이너리로 깨짐)
   국내 법무법인 실무 적용 자료 기준으로 확인했다. 핵심: ① 쿠키·행태정보 항목을
   나눠 각각 법적근거/수집항목/수집방법/수집목적/보유기간 표로 고지, ② 구글을
   위탁 표에 이름으로 명시, ③ 브라우저·OS별 옵트아웃 방법 안내, ④ 보관기간은
   최근 정부 개정 방향(6개월 이내 권고)에 맞춰 짧게. 위 3번의 주석 문구에 이미
   반영해뒀다. ⚠️ 정부 원문을 직접 대조한 게 아니라 실무 자료 기준이라, 광고를
   실제로 켜기 전에 한 번 더 확인하는 게 안전하다.
...
| `AppPromo` | 앱 추천 — **글이 끝난 뒤(출처 박스 다음)**. 오너 지시. frontmatter `hideAppPromo: true` 로 뺄 수 있다(본문과 정직하게 안 맞는 글용, 2026-08-09) |
| `Term` | 전문용어 인라인 툴팁("자세히 알아보기"는 용어사전 앵커로). ⚠️ `href` 의 앵커는 **손으로 짐작해서 적지 말 것** — `#이상운동증운동-변동온-오프이-뭔가요`처럼 원래 제목에 있던 하이픈(`온-오프`)이 빠지면 클릭해도 용어사전 맨 위로만 가고 설명까지 스크롤되지 않는다(2026-08-10 오너 발견). 새 Term 을 추가하면 `npm run build` 후 `dist/ko/lifestyle/glossary/index.html` 에서 실제 `id="..."` 를 grep 해 대조할 것 |
| `StoryHead` | (소식 전용) 소재 제목 — 큰 헤드라인 + "소식 N" 라벨 + 구분선 |
| `SourceQuote` | (소식 전용) 원문 발췌 인용 + "OO 원문 보기" 링크 박스 |
| `Callout` | 문장 단위 녹색 배경 강조 박스 |

- ⚠️ **본문 여백은 요소마다 위·아래를 같은 값으로 짝지어 준다.** 예전엔 모든 요소가
  `margin-top` 만 갖고 있어서 **아래 간격을 "다음에 오는 것"이 정했다** — 같은 회색 인용
  박스인데 아래가 문단이면 14px, 표·강조박스면 18px, 제목이면 44px 였다(오너 지적
  2026-08-14). 세로 여백은 인접 형제끼리 합쳐지므로(둘 중 큰 값), 요소가 스스로 위·아래를
  들고 있으면 옆에 무엇이 오든 그 값이 유지된다. **제목(h2·h3)만 일부러 비대칭**이다.
  `scripts/check-viewport.mjs` 가 빌드마다 실측해서 어기면 배포를 막는다.
- ⚠️ **MDX에서 한 줄을 `<b>…</b>` 하나로만 쓰지 말 것.** 그 줄이 문단(`<p>`)으로 안 감싸져
  날것 HTML 블록이 되고, 여백이 전부 `<p>` 에 걸려 있어 **위 여백이 0** 이 된다(앞 박스에
  달라붙는다). 앞뒤에 보통 글자를 한 자라도 두거나 `<p>` 로 감쌀 것.
  `scripts/check-built-html.mjs` 가 잡는다.
...
- **허브 목록(생활 요령·제도·지원·소식)의 해시태그 글자색은 초록(`var(--primary)`)**이다
  (오너 지시 2026-08-10, 이전엔 회색 `--text-sub`). 상세페이지에는 해시태그가 안 뜬다.
...
- 색 박스 콜아웃(주황 배경 + 굵은 라벨)은 쓰지 않는다 — "딱 봐도 AI가 만든 티"라는 오너 지적(2026-08-06).
  ...
  ⚠️ **2026-08-10에 `Callout`(초록 배경 강조 박스)을 새로 만들어 오너 승인을 받았다** —
  위 규칙과 다른 게 아니라, 색만 브랜드 그린(`--secondary-container`)이고 아이콘·"⚠️ 주의"
  같은 라벨을 안 붙인 게 다르다. 라벨·아이콘을 붙이는 순간 다시 이 규칙에 걸릴 수 있으니
  새로 손댈 때 주의할 것.
- ⚠️ **컴포넌트에 `<h1>~<h6>`·`<p>`·`<a>`·`<ul>`·`<ol>`·`<li>`·`<strong>`·`<blockquote>`·`<table>`
  태그를 그대로 쓰지 말 것.** `.article-body` 에는 마크다운 본문용 전역 스타일이 태그
  기준으로 걸려 있어서(`[category]/[slug].astro` 참고), 컴포넌트가 같은 태그를 쓰면 그
  전역 스타일이 우선순위로 이겨서 새어 들어온다(2026-08-09, `Steps.astro`: 카드 제목에
  `<h3>`를 썼다가 본문 소제목 스타일이 새어 들어와 번호와 제목 사이에 28px 틈이 생김 +
  `<ol>`/`<li>`를 썼다가 왼쪽에 22px 빈 공간이 생김 — 둘 다 오너가 스크린샷으로 발견).
  `<div>`(필요하면 `role="list"`/`role="listitem"`으로 접근성 보완)로 대체할 것. 진짜
  하이퍼링크(`<a href>`)처럼 태그를 바꿀 수 없는 경우엔 조상 클래스까지 selector에 넣어
  specificity 를 확실히 이기게 할 것(`SourceQuote.astro` 의 `.source-quote .quote-link` 참고).
...
## 소식(뉴스) → 앱 정보·나눔 피드 동기화 (2026-08-10)

오너 지시: 앱에 있던 "뉴스 크롤링" 기능(설계문서만 있고 실제로는 만들어진 적이 없었음,
`parkinon-app/docs/agents/AGENT_10_crawling.md` 삭제됨)을 대신해, 이 사이트의 소식 글을
앱 커뮤니티(정보·나눔)에 진짜 게시글로 밀어넣는다. 앱 쪽 `posts` 테이블엔 이미
`is_news`/`news_url`/`author_name_override` 컬럼이 준비돼 있었지만 아무도 안 쓰고 있었다
— 그래서 별도 `news_feed` 테이블(죽은 테이블, 좋아요/댓글 걸면 FK 에러 나던 구조) 대신
이 컬럼을 그대로 쓰기로 방향을 바꿨다.

- **스크립트**: `site/scripts/sync-news-posts.mjs` (`npm run sync-news --prefix site`)
- `src/content/articles/ko/news/*.mdx` 를 읽어 `Term`/`SourceQuote`/`Callout`/`StoryHead`/`<b>`
  같은 웹 전용 컴포넌트를 순수 마크다운으로 풀어쓰고(react-native-markdown-display 는 raw HTML을
  렌더링하지 않음), 사이트 내부 상대링크(`/ko/...`)는 절대 URL로 바꾼다(앱 안에서 상대경로로는
  못 열림).
- 작성자는 전용 시스템 계정 "파킨온"(`auth.users`+`public.users`, id
  `c5efd930-1c58-4241-8430-851ebb044442`) — 지인 테스트 계정을 빌려 쓰지 않는다.
- 히어로 이미지는 앱이 커뮤니티 사진을 올릴 때 쓰는 **같은** `r2-upload` 엣지함수 경로로
  올린다(봇 계정 로그인 → presigned URL → PUT). 별도 R2 자격증명 불필요.
- upsert 기준은 `posts.news_url` — 이미 올라간 소식 글을 고쳐도 다음 실행에 제목·본문·썸네일이
  갱신된다(새 게시글이 중복 생성되지 않음).
- 프론트매터 `tag`("파킨온 소식 #N")도 `posts.news_tag` 컬럼에 그대로 동기화되어 앱 목록
