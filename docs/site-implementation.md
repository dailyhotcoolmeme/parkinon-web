...
8. **여백·간격 값은 이 문서에 정한 값만 쓴다 — 새 페이지·새 컴포넌트를 만들 때마다 숫자를
   새로 지어내지 말 것.** (2026-08-08, 클리닉 페이지 검색결과·필터·목록 여백이 페이지마다
   제각각이라 오너가 크게 격노한 뒤 전수조사해서 확정.)
   - **시작점(히어로·제목 위치)**: 이미 3번 항목의 `.hub-hero-full` 시스템으로 news/lifestyle/
     institutions/exercise/clinical 전부 통일돼 있다. 새 허브 페이지도 이 시스템을 그대로 쓸 것 —
     새로 여백을 재지 않는다.
   - **빵부스러기 밑 여백**: `18px`. 모든 페이지 `.breadcrumb{margin-bottom:18px}`로 통일.
   - **필터/탭 행(허브 페이지 히어로 밑에 오는 국가·카테고리 선택 줄)**: `gap:22px; margin:22px 0 30px`.
     lifestyle·institutions·exercise·clinical(국가탭)이 이미 이 값을 쓴다 — 새 필터 행도 이 값.
   - **목록(세로로 쌓이는 카드·행 목록)의 항목 사이 간격 = 총 48px.** news/lifestyle/institutions는
     `.hub-row{padding:24px 0}`(위아래 24+24)로 이미 48px. 카드가 자체 `padding`을 갖는 경우
     (클리닉 `.trial-card`/`.research-card`의 `padding:20px 0`)는 목록의 `gap`을 `8px`로 둬서
     20+8+20=48px를 맞춘다 — **gap 값을 카드 padding과 따로 대충 정하지 말고, 항상 "카드
     padding×2 + gap = 48"이 되게 계산해서 넣을 것.**
   - **독립된 안내문구 한 줄**(카드 목록 시작 바로 앞, 예: `.lang-notice`/`.research-tagline`/
     `#searchResultCount`): **위쪽 여백 = 아래쪽 여백**, 값은 `24px`. 인접 요소(위는 박스, 아래는
     목록처럼 서로 다른 종류)에 맞춰 위·아래를 따로 정하지 말 것 — 그 요소 자체의 위·아래를 먼저
     같게 만들고, 필요하면 인접 요소 쪽 margin을 조정해서 그 값에 맞춘다. `line-height`도 반드시
     `1.6`으로 명시할 것(선언을 빼먹으면 브라우저 기본값이 다른 보조문구와 달라진다).
   - **카드 내부 텍스트 줄 간격**: `8px`(같은 블록 안 짧은 텍스트 줄 사이, 예: 제목→부제)와
     `12px`(summary 밖 새 내용 블록의 시작, 예: view-link·dl·abstract) 둘만 쓴다 — 클리닉 페이지
     `<style>` 상단 주석 참고, 카드형 UI를 다른 페이지에 새로 만들 때도 이 두 값을 그대로 쓸 것.
   - **아직 통일 안 한 것(그리드형 카드)**: exercise `.video-card`(`gap:40px 18px` PC/`32px 12px`
     모바일)와 tools `.tool-card`(`gap:18px`)는 세로 목록과 성격이 달라(그리드, row/column gap
     분리) 이번엔 손대지 않았다 — 두 페이지의 row-gap 차이(40 vs 18)가 실제로 문제인지는 시각
     확인 후 오너 판단이 필요하다.
