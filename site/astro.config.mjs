...
export default defineConfig({
  ...
  integrations: [
    ...
    sitemap({
       ...
       *
       * ⚠️ /tools 는 2026-08-10~09-07 여기서도 뺐었다("열지 말지 추후 결정" 상태,
       * nav.ts 참고). 그런데 robots.txt 는 막고 sitemap 에도 없는 채로 구글이 이 URL 을
       * 다른 경로로 찾아내 크롤했고, Search Console 이 "robots.txt에 의해 차단됨"을
       * 반복 알림으로 보냈다(2026-09-06 밤). 오너 지시(2026-09-07): 열어라 — 색인 허용.
       * robots.txt.ts 의 Disallow 도 같이 지웠다. nav.ts(헤더·푸터 노출)는 안 건드렸다 —
       * 메뉴 노출은 별개 결정이다.
       *
