...
### 번역본은 한국어 원본의 값을 물려받는다 (2026-08-13)

**같은 글을 언어마다 다시 판정하지 않는다.** 한국어 값은 오너가 직접 보고 승인한 것이고
(2026-08-12, 14편), 번역본은 같은 글이라 주제도 같다.

이걸 안 하면 어떻게 되는지 실제로 대조해 봤다 — 일본어판을 키워드로 다시 판정했을 때
**11편이 한국어판과 다른 화면**으로 갈렸다(`bed-mobility-tips` ko=family/ja=기본,
`orthostatic-hypotension-tips` ko=effectTracking/ja=기본 …). 한국어 "진료"에 해당하는
일본어 표현이 `受診`·`外来`·`通院`·`診察` 로 갈라져서 키워드로는 같은 판정이 안 나온다.

물려받을 원본이 **없을 때만** 키워드로 판정한다:
- 한국어 원본에 `appFeature` 가 비어 있는 글
- 그 언어 전용 글 (일본 제도 글처럼 한국어 원본이 아예 없는 글)

⚠️ 원본에 값이 있어도 **그 언어판 앱에 없는 기능은 물려받지 않는다.**
`young-onset-parkinsons` 는 한국어판이 `community` 인데, 해외판 앱에는 커뮤니티 탭이
아예 없다 — 없는 기능을 광고하게 되므로 떨어뜨리고 기본 화면으로 둔다.
(오너가 크게 지적한 지점이다. 스크립트가 그 사실을 로그로 남긴다.)

### 그 외 동작 방식과 **일부러 그렇게 만든 제약**

1. **제목·설명·세 줄 요약만 본다.** 본문까지 훑으면 용어 툴팁(`<Term brief>`)의 무관한
   의학 설명이 걸린다 — 실제로 세포치료 글이 "sleep" 에 걸렸다.
2. **소식 설명문의 상투구를 뺀다.** "환자분·보호자분 관점에서 정리했습니다",
   일본어 「患者さんとご家族の視点で」 같은 문구가 모든 소식에 들어가서, 빼지 않으면
   전부 `family` 로 잘못 잡힌다. **언어를 추가할 때 그 언어의 상투구도 같이 넣을 것.**
3. **"운동" 단독은 키워드로 쓰지 않는다.** 한국어 "운동"은 대부분 "운동 증상(motor
   symptoms)" 이고, 일본어도 `運動症状`·`非運動症状` 이 흔하다. `유산소`·`걷기`·`운동을`,
   `有酸素`·`運動を`·`運動が` 처럼 신체활동을 가리키는 형태만 쓴다.
...
양쪽 언어 모두 건드리지 않았다. 상속을 넣은 뒤에는 일본어 19편이 한국어판과 같은 값을
물려받고, `young-onset-parkinsons` 의 `community` 만 정확히 떨어졌다.
