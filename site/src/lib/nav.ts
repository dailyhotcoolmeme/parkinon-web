...
const AVAILABLE: Record<string, readonly NavKey[]> = {
   ...
   * - news         : 연다 — 2026-09-02 영어판 9편을 전부 스페인어로 옮겼다. 처음 열 때
   *                  잠깐 뺐다가 오너에게 크게 지적받았다("제일 중요한 소식을 빼먹고
   *                  뭐하는건데!!"). 소식은 이 사이트의 중심 축이다 — 번역이 늦으면
   *                  번역을 하는 것이지 축을 빼는 것이 아니다.
   * - institutions : 2026-09-02 에 열었다. **미국(스페인어) 11편** — 영어판 미국 글을
   *                  옮긴 것이다. 미국 히스패닉 5~6백만 명이 대상이고, 앱이 이미 출시된
   *                  시장이라 계획서에서 제도 축 1순위였다. 중남미(멕시코부터)는 다음.
   *                  ⚠️ 미국 제도명(Medicare·SSDI·Medicaid…)은 영어 그대로 둔다 —
   *                  독자가 실제로 마주치는 이름이고, es.medicare.gov 도 그렇게 쓴다.
   * - clinical     : 연다. TRIAL_COUNTRIES 에 멕시코·브라질·칠레·아르헨티나·콜롬비아·페루를
   *                  추가해서 중남미 독자가 자기 나라 시험을 실제로 볼 수 있다.
   */
  es: ['news', 'lifestyle', 'institutions', 'clinical'],
  /*
