 ...
 *   1) 동시 실행 수를 제한한다 — 언어가 늘어도 DB 부하가 따라 늘지 않는다.
 ...
 *
 * ⚠️ 2026-08-21 재조정: 위 값(동시 3 · 재시도 4)은 **언어가 3개이던 때** 기준이었는데
 * 프랑스어가 늘어 4개가 되면서 프로덕션 빌드가 4번 재시도를 다 쓰고 죽었다.
 * 그때 실측해 보니 **동시 실행이 전혀 없는 상태에서도 이 조회 한 건이 2~3초**였다
 * (EXPLAIN: 160행 인덱스 스캔인데 PK 조회 한 건당 10~17ms — abstract_en(jsonb)이 TOAST 에
 * 있어 행마다 별도 I/O 가 걸린다). 즉 쿼리 계획 문제가 아니라 인스턴스가 느린 것이라,
 * 동시에 여러 갈래를 던지면 그대로 statement timeout 에 걸린다.
 * → 동시 수를 낮추고(2) 재시도를 늘려(6) 총 대기 시간을 벌었다. 빌드가 몇십 초 느려지는
 *   대신 배포가 막히지 않는다. 언어를 또 추가할 때 이 값을 다시 볼 것.
 */
